...
def graphics_elbow_silhouette(x, randon_state=42, intervalo_k=(2,11)):
    ...
    for i in k_range:
        kmeans = KMeans(n_clusters=i, random_state=randon_state, n_init=10)
        kmeans.fit(x)
...
def plot_cluster_2D(
    ...
    for i, centroid in enumerate(centroids):
        ...
        if ligar_centrois:
            ...
            ax.text(*centroid,f"{i}")

        if ligar_pontos:
            s = ax.scatter(x,y, c=column_clusters, cmap=cores)
            ax.legend(*s.legend_elements(),bbox_to_anchor=(1.3,0.8) )
...
def plot_columns_percent_hue_cluster(
    ...
    for ax, column in zip(axs.flatten(), columns):
        ...
        if dataframe[column].dtype != "object":
            h.set_xticks(range(dataframe[column].nunique()))
        ...
        for bars in h.containers:
            h.bar_label(
                bars,
                label_type="center",
                labels=[f'{b.get_height():.1%}' for b in bars], color="white", weight="bold")
